...
import { emailService, isValidEmail } from './server/email';
...
app.post('/api/quotes', async (req: Request, res: Response) => {
  try {
    ...
    try {
      console.log(`[Quote] Dispatching quote request for ${result.quote_reference} to sales team...`);
      const emailResult = await emailService.sendQuoteNotifications({
      ...
      if (!emailResult.salesEmailSent) {
        console.error(`[Quote Email Error] Sales notification could not be delivered: ${emailResult.error}`);
        return res.status(500).json({
          success: false,
          message: 'Your request could not be submitted at this time. Please try again or contact DK Medical directly.'
        });
      }
    } catch (emailErr) {
    ...
    return res.status(201).json({
      ...
      message: 'Thank you for your enquiry our sales team will get back to you shortly.'
    });
...
app.post('/api/contact', async (req: Request, res: Response) => {
  try {
    ...
    // Send request to be contacted directly to sales team (sales@dkmedical.co.za)
    try {
      console.log(`[Contact] Dispatching request to be contacted for inquiry ${result.reference} to sales team...`);
      const emailResult = await emailService.sendContactInquiryNotifications({
      ...
      if (!emailResult.salesEmailSent) {
        console.error(`[Contact Email Error] Sales notification could not be delivered: ${emailResult.error}`);
        return res.status(500).json({
          success: false,
          message: 'Your request could not be submitted at this time. Please try again or contact DK Medical directly.'
        });
